    ...
    <div>
          ...
          <div class="bd">
            <div class="content overview">
              <ul>
                ...
                <li>配售已收款数量：<span>{{historyAggregateData.placingPaymentNum}}</span></li>
                <li>配售未收款数量：<span>{{historyAggregateData.placingNoPaymentNum}}</span></li>
              </ul>
            </div>
          </div>
        ...
        <div class="section">
          ...
          <div class="bd">
            <div class="content overview">
              <ul>
                <li>当日成交总额：<span>{{aggregateData.todayTurnoverTotalMoney}}万</span></li>
                <li>当日成交总量：<span>{{aggregateData.todayTurnoverTotal}}</span></li>
                <li>客户成交总额：<span>{{aggregateData.todayIndividualTurnoverTotalMoney}}万</span></li>
                <li>客户成交总量：<span>{{aggregateData.todayIndividualTurnoverTotal}}</span></li>
                <li>持有原始商品：<span>{{aggregateData.holdBaseProductCount}}</span></li>
        ...
        historyAggregate: function(){
          ...
          this.$http.get('/api/historyTradeTotal',{params:{ session_token: this.session_token, productNo: this.defaultProductId }}).then((response) => { 
            ...
            if(response.body.result == ERR_OK){ 
              ...
              this.historyAggregateData.placingPaymentNum = this.toolHelper.formatNum(parseInt(this.historyAggregateData.placingPaymentNum));
              this.historyAggregateData.placingNoPaymentNum = this.toolHelper.formatNum(parseInt(this.historyAggregateData.placingNoPaymentNum));
            }else{
              alert(response.body.message);
            }
        ...
        aggregate: function(date, dateValu){
          ...
          this.$http.get('/api/tradeTotal',{params:{ session_token: this.session_token, productNo: this.defaultProductId, date: date}}).then((response) => { 
            ...
            if(response.body.result == ERR_OK){ 
              ...
              this.aggregateData.todayTurnoverTotalMoney = this.toolHelper.formatNum(parseInt(this.aggregateData.todayTurnoverTotalMoney/10000));
              this.aggregateData.todayTurnoverTotal = this.toolHelper.formatNum(parseInt(this.aggregateData.todayTurnoverTotal));
              this.aggregateData.todayIndividualTurnoverTotalMoney = this.toolHelper.formatNum(parseInt(this.aggregateData.todayIndividualTurnoverTotalMoney/10000));
              this.aggregateData.todayIndividualTurnoverTotal = this.toolHelper.formatNum(parseInt(this.aggregateData.todayIndividualTurnoverTotal));
              this.aggregateData.holdBaseProductCount = this.toolHelper.formatNum(parseInt(this.aggregateData.holdBaseProductCount));
              this.aggregateData.individualJoinNum = this.toolHelper.formatNum(parseInt(this.aggregateData.individualJoinNum));
              if(date == ''){
